...
Formatted, versioned legislative data for Ohio. Part of the [govbot](https://github.com/chihacknight/govbot) civic data project.
...
This is the **formatted data repo** for Ohio — one of two repos per jurisdiction:

1. **Scraper repo** (`govbot-openstates-scrapers/oh-legislation`) — runs [OpenStates](https://github.com/openstates/openstates-scrapers) scrapers, produces raw scraped JSON
2. **This repo** (`oh-legislation`) — reads the scraper repo's output and:
...
Current settings for Ohio:

- **State code**: `oh`
- **Toolkit branch**: `main`
- **Scraper repo**: `govbot-openstates-scrapers/oh-legislation`
...
Pulls the latest scraped data from the scraper repo, sanitizes it, formats it into `country:us/state:oh/sessions/...`, links events, and updates orphan tracking.
...
├── country:us/
│   └── state:oh/            # state:usa for federal, state:il for Illinois, etc.
│       └── sessions/
│           └── {session_id}/
│               ├── bills/
│               │   └── {bill_id}/
...
Formatted metadata is saved to `country:us/state:oh/sessions/`, organized by session and bill.
...
```json
{
  "HB999": {
    ...
    "vote_count": 2,
    "event_count": 0,
    "path": "country:us/state:oh/sessions/103/bills/HB999"
  }
}
```
...
- Check the scraper repo (`govbot-openstates-scrapers/oh-legislation`) actually got fresh data first — this repo only formats what that repo has already scraped
